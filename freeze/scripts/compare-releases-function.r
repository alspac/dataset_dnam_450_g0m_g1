compare.releases <- function(mapping, x.dir, y.dir) {
    ...
    y.samplesheet <- y.samplesheet[which(y.samplesheet$ALN %in% mapping$new),,drop=F]
    if (nrow(y.samplesheet) == 0)
        stop("mapping$new does not match ALN's in the second dataset")
    
    y.samplesheet$cid <- mapping$aln[match(y.samplesheet$ALN, mapping$new)]
    ...
    if (length(ids) == 0)
        stop("there is something with the ALN mapping")

    x.samplesheet <- x.samplesheet[match(ids, x.id),,drop=F]
    y.samplesheet <- y.samplesheet[match(ids, y.id),,drop=F]
	...
	cat(name,"\n")
        ...
        if (!compare.data.frames(x.counts, y.counts))
            warning(paste("Cell count estimates", name, "do not match"))
